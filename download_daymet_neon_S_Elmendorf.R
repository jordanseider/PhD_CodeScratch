...
read_daymet_dir <- function(dest_dir) {
  ...
  if ("date" %in% names(df)) {
    ...
    any(c("yday", "doy", "day_of_year", "dayofyear") %in% names(df)) &&
      "year" %in% names(df)
  ) {
    doy_col <- intersect(
      c("yday", "doy", "day_of_year", "dayofyear"),
      names(df)
    )[1]
    df <- df |>
      mutate(
...
compute_heatwaves <- function(
  df,
  threshold_pct = 99,
  min_days = 5,
  ref_start = 1991,
  ref_end = 2020,
  tmax_candidates = c("tmax", "tmax_c", "tmax.C", "TMAX", "tmax..deg.c.")
) {
  # detect tmax column
  tmax_col <- names(df)[tolower(names(df)) %in% tolower(tmax_candidates)]
  if (length(tmax_col) != 1) {
    stop(
      "No tmax column found in data. Expected one of: ",
      paste(tmax_candidates, collapse = ", ")
    )
  }
  tmax_col <- tmax_col[1]
  ...
  if (!"date" %in% names(df)) {
    stop("date column missing; ensure read_daymet_dir produced a date column.")
  }
  if (!"siteCode" %in% names(df)) {
    stop("siteCode column missing.")
  }
...
plot_heatwaves_timeseries <- function(
  hw_results_or_df,
  ...
  parse_xrange_to_years <- function(xr) {
    if (length(xr) != 2) {
      stop("xrange must be length 2 (start, end)")
    }
    if (inherits(xr, "Date")) {
    ...
    if (is.character(xr)) {
      ...
      stop(
        "xrange character values must be parseable as Dates or numeric years"
      )
    }
    if (is.numeric(xr)) {
      return(as.numeric(xr))
    }
  ...
  if (x_limits[1] > x_limits[2]) {
    stop("xrange start must be <= end")
  }

  # obtain combined dataframe
  combined <- if (
    is.list(hw_results_or_df) && "combined" %in% names(hw_results_or_df)
  ) {
  ...
  # basic checks
  if (!"siteCode" %in% names(combined)) {
    stop("siteCode column missing in input data.")
  }
  if (!"date" %in% names(combined)) {
    stop("date column missing in input data.")
  }
  if (!"doy" %in% names(combined)) {
    combined <- combined |> mutate(doy = lubridate::yday(date))
  }
  if (!"is_heatwave_day" %in% names(combined)) {
    combined <- combined |> mutate(is_heatwave_day = FALSE)
  }
  if (!"heatwave_id" %in% names(combined)) {
    combined <- combined |> mutate(heatwave_id = NA_character_)
  }
